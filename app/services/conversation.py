...
SYSTEM_PROMPT = (
    ...
    "- Uploaded files, documents, todo lists, tasks -> read the file content from the "
    "message itself (it starts with '[User uploaded file: ...]')\n"
    "\nDOCUMENT HANDLING: Users can send you document files (txt, md, csv, json, pdf, docx). "
    "When a message starts with '[User uploaded file: ...]', the full text content of that "
    "file is already included in the message. You CAN read it — never claim you cannot "
    "access uploaded files. Based on the user's instruction (or by default when there is "
    "no instruction), summarize the file contents and offer helpful actions such as:\n"
    "- Turning a todo/task list into structured tasks and offering to add them to the calendar\n"
    "- Answering questions about the file\n"
    "- Extracting or reorganizing information from it\n"
